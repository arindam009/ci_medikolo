...
/*================================ Edit recipe ===========================*/
...

    {
		...
		
		{
			extract($_POST);
			extract($_FILES);
			$delete_instruction=$this->Recipe_model->delete_data_by_recipecode($id,'tbl_recipe_instruction');
			for($i=0;$i<count($inst_name);$i++){				
				if($_FILES['ins_image']['name'][$i]!='' || $_FILES['ins_image']['name'][$i]!=null ){	
					$this->load->library('upload');
				$_FILES['userfile']['name']=$_FILES['ins_image']['name'][$i];
				$_FILES['userfile']['type'] = $_FILES['ins_image']['type'][$i];
                $_FILES['userfile']['tmp_name'] = $_FILES['ins_image']['tmp_name'][$i];
                ...
                if (! $this->upload->do_upload()) {
					...
					$instruction['images']= $prev_link[$i];
				}				
					$instruction['recipe_code']=$id;
					$instruction['name']=$inst_name[$i];
					$data_instuction_insert = $this->Recipe_model->instruction_insert($instruction);
			...
			$delete_ingredients=$this->Recipe_model->delete_data_by_recipecode($id,'tbl_recipe_ingredients');
			for($ig=0;$ig<count($ing_name);$ig++){
				$ingradients['recipe_code']=$id;
				$ingradients['name']=$ing_name[$ig];
				$ingradients['quantity']=$ing_quantity[$ig];
				$ingradients['unit']=$ing_unit[$ig];
				$data_ingradients_insert = $this->Recipe_model->ingradients_insert($ingradients);
			...
			//insert nutrituioin element
			$delete_ne=$this->Recipe_model->delete_data_by_recipecode($id,'tbl_recipe_nutritional_elements');
			for($ne=0;$ne<count($ne_name);$ne++){
				$nutrition['recipe_code']=$id;
				$nutrition['name']=$ne_name[$ne];
				$nutrition['quantity']=$ne_quantity[$ne];
				$nutrition['unit']=$ne_unit[$ne];
				$data_nutrition_insert = $this->Recipe_model->nutrition_insert($nutrition);
			...
			// exit();
			 ...
			 $this->form_validation->set_rules('description', 'Description', 'trim|required');
                    ...
                    // Faking upload calls to $_FILE
                        ...
                        // print_r($error);die();                        
                            ...
                            $final_files_data_recipe[] = $this->upload->data();                           
                             ...
                             $data['feature_image']= $final_files_data_recipe[0]['file_name'];
                              unlink("../uploads/recipe_image/".$this->input->post('feature_image_prev_link'));
			   ...
			   if ($this->form_validation->run() == TRUE) {
                  ...
                  $data_inserted = $this->Recipe_model->edit_data_by_recipecode($data,$id);
...
/*================================ Delete recipe===========================	
    ...
    public function week($id)
    {
        ...
        {
               $data['recipe_type'] = 'recipe of the week';
               $data_inserted = $this->Recipe_model->edit_data($data,$id);
               $this->session->set_flashdata('success_msg', 'Data edited Successfully'); 
               redirect($_SERVER['HTTP_REFERER']);              
        }
    ...
    public function month($id)
    {
        ...
        {
               $data['recipe_type'] = 'recipe of the month';
               $data_inserted = $this->Recipe_model->edit_data($data,$id);
               $this->session->set_flashdata('success_msg', 'Data edited Successfully'); 
               redirect($_SERVER['HTTP_REFERER']);              
        }

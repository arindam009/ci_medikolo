...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		$this->_ci->output->set_header('Expires: Sat, 07 Jan 2017 00:00:01 GMT');
		...
		if ($this->_layout)
		{
			//print_r(self::_find_view_folder());
	...
	public function title()
	{
		...
		if ($title_segments = func_get_args())  // change by debmalya for (Only variables should be assigned by reference) //removed &
		{
			$this->_title = $title_segments[1];//implode($this->_title_separator, $title_segments);
		}
